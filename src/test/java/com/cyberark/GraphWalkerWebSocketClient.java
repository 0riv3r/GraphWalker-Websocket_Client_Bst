...
public class GraphWalkerWebSocketClient {
    ...
    /**
     * Creates an instance of the GraphWalker WebSocket client. No connections will
     * be made until the {@link #run() run} method is called. The default hostname
     * is localhost. The default port is 8887
     */
    ...
    /**
     * Creates an instance of the GraphWalker WebSocket client. No connections will
     * be made until the {@link #run() run} method is called.
     *
    ...
    /**
     * Connects the GraphWalker client to the GraphWalker WebSocket server on
     * ws://hostname:port When connected, a GraphWalker machine will be created on
     * the server which will serve this client only.
     */
    public void run() {
        ...
        try {
            wsc = new WebSocketClient(new URI("ws://" + host + ":" + port), new Draft_6455()) {
                ...
                public void onMessage(String message) {
                    ...
                    cmd = false;
                    String type = root.getString("command").toUpperCase();
